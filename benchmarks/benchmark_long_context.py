...
Configs (ordered by importance — longest first):
  ...
  K) Wave Field   seq=2048  — speed crossover zone
  L) Standard     seq=2048  — speed crossover zone
  A) Wave Field   seq=512   — short-context reference
  B) Standard     seq=512   — short-context reference

All Wave configs: V4.3.9 (analytic kernel, 2L FM, QK LR x3, frozen heads).
...
V4.3.9 env vars:
  FROZEN_HEADS=4  — freeze first 4 heads' damping (prevents bigram collapse)
  LOCAL_WINDOW=32 — enable local window softmax attention

Sweet spot:  CONFIGS=K,L,C,D  (2K + 4K — where O(n log n) advantage shows)
Quick run:   CONFIGS=K,L       (~1 hour on RTX 3060)
...
CONFIGS = [
    ...
    {'key': 'K', 'name': 'K) Wave Field seq=2048', 'type': 'wave',
     'seq_len': 2048, 'batch_size': 4, 'field_size': 4096},
    {'key': 'L', 'name': 'L) Standard seq=2048', 'type': 'standard',
...
def get_results_dir():
    """Get versioned results directory. Falls back to results/ if version unknown."""
...
def train_run(model, train_ids, val_ids, vocab_size, device, cfg, use_amp):
    ...
    run_tag = f"{cfg['type']}_seq{seq_len}"
...
def main():
    ...
    # Save results — both to base results/ and versioned dir
    ...
    # Save to versioned dir (primary)
    vdir_data = os.path.join(vdir, 'data')
    os.makedirs(vdir_data, exist_ok=True)
    v_path = os.path.join(vdir_data, 'long_context.json')
    with open(v_path, 'w') as f:
        json.dump(all_results, f, indent=2)

    # Also merge into base results/long_context.json (backwards compat)
    results_path = os.path.join(base_results, 'long_context.json')
    existing = []
    ...
    print(f"  LONG-CONTEXT RESULTS ({budget_str} tokens, WikiText-2) {version_str}")
